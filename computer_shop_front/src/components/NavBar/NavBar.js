	...
	return <header className='navBar' style={location.pathname !== "/" || user.level === 1 || user.level === 2 ? {height: "50px"} : {}}>
		...
		>
			...
			<div className='accountPopupButtons'>
				{isNoUser
					?<>
						<button className='accountButton' onClick={()=>{sessionStorage.setItem("lastUrl", location.pathname);open("/login")}}>Log in</button>
					...
					:<>
						...
						{user.level === 1
						 ?<>
							<button className='accountButton' onClick={()=>open(`/supplier/${user._id}`)}>Your Page</button>
						 </>
						 :user.level === 0
							? <>
